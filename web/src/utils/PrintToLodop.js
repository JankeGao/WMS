...
PrintToLodop.prototype.g_text = function (control) {
  ...
  //   LODOP.ADD_PRINT_TEXT(tdata.y + "px", tdata.x + "px", tdata.width + "px", tdata.height + "px", content.substring(0, 25) + '\n' + content.substring(25));
  // }
  // else {
...
PrintToLodop.prototype.g_image = function (control) {
  ...
  // // LODOP.ADD_PRINT_BARCODE(control.data.y, control.data.x, control.data.width, control.data.height, imgdata);
  // LODOP.ADD_PRINT_BARCODE(control.data.y, control.data.x, control.data.width, control.data.height, "QRCode", content);
...
PrintToLodop.prototype.print = function () {
  ...
  // LODOP.SET_PRINT_PAGESIZE(1, 24, 72, "")
...
PrintToLodop.prototype.definedPrint = function (Name) {
  if (!this.load) {
    alert(this.msg);
    return;
  }
  ...
  // LODOP.PREVIEW(); 打印预览
  console.log(Name)
  LODOP.SET_PRINTER_INDEX(Name);// 使用自定义打印机
  LODOP.PRINT();
  // LODOP.PRINT_DESIGN() 打印设计
}

export default PrintToLodop;