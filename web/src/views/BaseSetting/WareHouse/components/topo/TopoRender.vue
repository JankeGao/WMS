...
import { GetWareHouseList } from '@/api/PTLProgress'

import { getWareHousePlan } from './TopoControls'
...
export default {
  ...
  mounted() {
    ...
    getWareHousePlan('03').then((res) => { // 从后台数据库获取仓库平面图数据
      // var resData = JSON.parse(res.data.Content)
      ...
      this.configData = resData
      // if (this.Task.EmptyTask === true) {
      //   this.timer()
      // } else {
      //   clearTimeout(this.timer())
      // }

      // console.log(this.Task.EmptyTask)
    ...
    // console.log(this.configData.layer)
    // alert(this.configData)
  },
  methods: {
    ...
    getWareHouseList() {
      GetWareHouseList().then(response => {
        ...
        this.wareHouseList = usersData
      })
    },
    handleWareHouseChange() {
      getWareHousePlan(this.WareHouseCode).then((res) => { // 从后台数据库获取仓库平面图数据
        console.log(res)
        var da = JSON.parse(res.data.Content)
        if (da.Success) {
          var resData = JSON.parse(da.Message)
