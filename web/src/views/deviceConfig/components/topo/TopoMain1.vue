      ...
      >
        <template v-for="(component,index) in configData.components">
          <div
            ...
            @keydown.ctrl.89.stop="redo"
          >
            <component :is="parseView(component)" :ref="'comp' + index" :detail="component" :edit-mode="true" :selected="selectedComponentMap[component.identifier]?true:false" />
...
// import { checkInRange, checkByPointInRect, checkByRectCollisionDetection } from '@/assets/libs/topo'
...
export default {
  ...
  methods: {
    ...
    execute(conmand, component) {
      this.$store.dispatch('topoEditor/execute', conmand, component)
    },
    format(percentage) {
    ...
    onLayerMouseup(event) {
      if (this.flag.startsWith('resize')) {
        ...
        conmand.dx = dx
        conmand.dy = dy
        conmand.items = this.selectedComponentMap
        this.execute(conmand)
      }
      this.flag = ''
    },
